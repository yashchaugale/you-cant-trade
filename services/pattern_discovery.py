...
def _pattern_values(trade: dict[str, Any]) -> list[tuple[str, str]]:
    ...
    if (
        ...
        values.append(
            (
                "setup_regime",
                f"{setup.strip()} + {regime.strip()}",
            )
        )

    if (
        isinstance(structure_state, str)
        and structure_state.strip()
        and isinstance(session, str)
        and session.strip()
    ):
        values.append(
            (
                "structure_session",
                f"{structure_state.strip()} + {session.strip()}",
            )
        )
    ...
    if (
        ...
        values.append(
            (
                "direction_session",
                f"{direction.strip()} + {session.strip()}",
            )
        )
    ...
    if (
        ...
        values.append(
            (
                ...
                f"{setup.strip()} + {session.strip()} + {regime.strip()}",
            )
        )
